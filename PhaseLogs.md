...
**Post-phase keypad fixes + audit (`bae67da` + follow-up):** the keypad shipped with collapsed digit rows (a wrapper lost its `flex: 1` in a refactor) and floated mid-screen (workout ScrollView lacked `flex: 1`) — both fixed. Dismissal added per owner request: Android back closes the keypad first (committing the draft), taps outside commit + dismiss (field taps still switch), iOS swipe-back disabled while open, Finish commits any open draft. Audit then found three more: (1) recovery dropped sessions with zero exercises, orphaning invisible `active` rows — empty sessions now recover; (2) editing a completed set's values never recomputed `est_1rm`, which would poison future PR detection — `updateSetValues` recomputes for completed non-warm-up sets; (3) tap-✓ with an empty value was a silent no-op after the keypad rewrite — it now redirects into the empty field, reading FRESH store state (the render closure predates the just-made commit).
